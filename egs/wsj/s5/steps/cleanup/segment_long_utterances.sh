...
for f in $data/feats.scp $text $extra_files $srcdir/model/tree \
  $srcdir/model/final.mdl $srcdir/model/cmvn_opts; do
  if [ ! -f $f ]; then
...
if [ $stage -le 3 ]; then
  ...
  cp $srcdir/model/final.mdl $dir
  cp $srcdir/model/frame_subsampling_factor $dir
  cp $srcdir/model/tree $dir
  cp $srcdir/model/cmvn_opts $dir
  cp $srcdir/model/{splice_opts,delta_opts,final.mat,final.alimdl} $dir 2>/dev/null || true
  cp $srcdir/data/lang/phones.txt $dir 2>/dev/null || true
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 1 \
            $data_uniform_seg pre-trained/extractor pre-trained/model/ivectors_source_split
  ...
  if [ -f $srcdir/trans.1 ]; then
    ...
    steps/cleanup/decode_segmentation_chain.sh \
      --beam $beam --lattice-beam $lattice_beam --nj $nj --cmd "$cmd --mem 4G" \
      --skip-scoring true --allow-partial false --num-threads 6 \
      $graph_dir $data_uniform_seg $decode_dir
  fi
fi
...
if [ $stage -le 5 ]; then
  steps/cleanup/internal/get_ctm.sh \
    ...
    --print-silence true --frame-shift 0.03 \
